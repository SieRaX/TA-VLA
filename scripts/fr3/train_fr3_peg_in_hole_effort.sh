#!/usr/bin/env bash
# TA-VLA pi0 LoRA finetune on Franka FR3 Peg_in_hole with torque (Dec-H + future-effort).
...
# "fr3/peg_in_hole_tavla_train_100" is a symlink to the physical
# Peg_in_hole/tavla/train_100 directory (HF repo_id allows only one slash).
...
SMOKE_NORM_STATS=assets/pi0_fr3_peg_in_hole_effort_smoke/fr3/peg_in_hole_tavla_train_100/norm_stats.json
if [ -f "$SMOKE_NORM_STATS" ]; then
    ...
    uv run -- python scripts/compute_norm_stats.py --config-name=pi0_fr3_peg_in_hole_effort_smoke
fi

echo "[2/4] smoke training (200 steps)..."
uv run -- python scripts/train.py \
    pi0_fr3_peg_in_hole_effort_smoke \
...
REAL_NORM_STATS=assets/pi0_fr3_peg_in_hole_effort/fr3/peg_in_hole_tavla_train_100/norm_stats.json
if [ -f "$REAL_NORM_STATS" ]; then
    ...
    uv run -- python scripts/compute_norm_stats.py --config-name=pi0_fr3_peg_in_hole_effort
fi

echo "[4/4] real training..."
uv run -- python scripts/train.py \
    pi0_fr3_peg_in_hole_effort \
