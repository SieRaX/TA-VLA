#!/usr/bin/env bash
# TA-VLA pi0 *full* finetune on Franka FR3 PnP_fixed with torque (DePost + future-effort).
# Variant: EffortType.EXPERT_IN_FUT, effort_dim=7, current-frame effort input.
#
# Differences from the LoRA twin (train_fr3_pnp_fixed_effort.sh):
...
SMOKE_NORM_STATS=assets/pi0_fr3_pnp_fixed_effort_full_smoke/fr3/pnp_fixed_tavla_train_100/norm_stats.json
if [ -f "$SMOKE_NORM_STATS" ]; then
    ...
    uv run -- python scripts/compute_norm_stats.py --config-name=pi0_fr3_pnp_fixed_effort_full_smoke
fi

echo "[2/4] smoke training (200 steps)..."
uv run -- python scripts/train.py \
    pi0_fr3_pnp_fixed_effort_full_smoke \
    --exp_name="smoke_$(date +%Y%m%d_%H%M%S)" \
    --overwrite

# --- Phase 2: real full finetune (batch=32, wandb on, FSDP across visible GPUs) ---------
REAL_NORM_STATS=assets/pi0_fr3_pnp_fixed_effort_full/fr3/pnp_fixed_tavla_train_100/norm_stats.json
if [ -f "$REAL_NORM_STATS" ]; then
    ...
    uv run -- python scripts/compute_norm_stats.py --config-name=pi0_fr3_pnp_fixed_effort_full
fi

echo "[4/4] real full-FT training..."
uv run -- python scripts/train.py \
    pi0_fr3_pnp_fixed_effort_full \
